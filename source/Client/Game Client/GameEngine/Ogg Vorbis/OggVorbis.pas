...
type
  ...
  HeaderInfo = record
    ...
    { Extended data for internal use }
    FileSize: Integer;                                    { File size (bytes) }
    ID3v2Size: Integer;                              { ID3v2 tag size (bytes) }
    Tag: array [0..VORBIS_FIELD_COUNT] of string;    { Vorbis tag information }
  end;
...
begin
  try
    ...
    FileMode := 0;
    Reset(SourceFile, 1);
    Seek(SourceFile, Header.ID3v2Size + 28);
...
var
  Iterator: Integer;
begin
  { Get first byte position of Vorbis tag info }
  Result := 0;
...
begin
  try
    ...
    FileMode := 0;
    Reset(SourceFile, 1);
    { Seek to Vorbis tag info }
    Seek(SourceFile, Header.ID3v2Size);
    BlockRead(SourceFile, Data, SizeOf(Data));
    Seek(SourceFile, Header.ID3v2Size + GetTagPosition(Data));
...
function GetSampleNumber(const FileName: string): Integer;
var
  SourceFile: file;
...
begin
  ...
  if FGetDuration > 0 then
    Result := Round((FFileSize - FID3v2.Size) / FGetDuration / 125);
end;
...
begin
  ...
  begin
    ...
    { Get Vorbis tag information and fill properties }
