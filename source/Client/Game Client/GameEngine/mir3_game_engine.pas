...
uses
  Classes, Windows, D3DX9, Direct3D9, DirectShow9, Bass, OpenJpeg;
...
const
  ...
  BLEND_DEFAULT          = 0;//BLEND_COLORMUL or BLEND_ALPHABLEND or BLEND_NOZWRITE;
...
uses
  Messages, Math, MMSystem, ShellAPI, SysUtils, Types,
  ZLib, ZipUtils, UnZip;
...
begin
  ...
  if (EventType = mir3_game_engine.INPUT_KEYDOWN) then
  begin
  ...
  if (EventType = mir3_game_engine.INPUT_KEYUP) then
  begin
  ...
  if (EPtr.Event.EventType = mir3_game_engine.INPUT_KEYDOWN) or (EPtr.Event.EventType = INPUT_MBUTTONDOWN) then begin
...
// Get adapter info
  ...
  FD3D.GetAdapterIdentifier(D3DADAPTER_DEFAULT,D3DENUM_WHQL_LEVEL{0},AdID);
...
//D3DCREATE_SOFTWARE_VERTEXPROCESSING
  HR := FD3D.CreateDevice(D3DADAPTER_DEFAULT,D3DDEVTYPE_HAL,FWnd,
